...
TError TBeamSolver::ParseLines(TInputLine *Lines,int N,bool OnlyParameters)
{
	...
	if (!BeamDefined) {
		S="ERROR: The beam energy and phase distributions are not defined. Put BEAM line in correct format into the input file";
		ShowError(S);
		Error=ERR_FORMAT;
	}
	if (!CurrentDefined) {
		S="ERROR: The beam current and transverse distribution are not defined. Put CURRENT line in correct format into the input file";
		ShowError(S);
		Error=ERR_FORMAT;
	}
	if (Ni<1) {
