...
//---------------------------------------------------------------------------
AnsiString TBeamSolver::GetWord(ifstream &f)   //Reads the next word from fstream
{
   AnsiString S;
   char s[MAX_CHAR];
...
//---------------------------------------------------------------------------
AnsiString TBeamSolver::GetLine(ifstream &f)   //Reads the next line from fstream
{
   AnsiString S;
   char s[MAX_CHAR];
...
//---------------------------------------------------------------------------
int TBeamSolver::NumWords(AnsiString &L)  //Counts number of words in the line
{
	int i=0,Nmax=0,N=0;
...
//---------------------------------------------------------------------------
AnsiString TBeamSolver::ReadWord(AnsiString &L,int N)   //Parses the Nth word from the string
{
	AnsiString S="";
	int i=1,Nmax=0,W=0;
...
/*        logFile=fopen("BeamSolver.log","a");
	...
	while (!fs.eof()){
		...
		S=ReadWord(L);  //Gets the first word in the line (should be a key word)
...
/*				if(S=="COMMENT"){
			...
			if (P==COMMENT)       //comment length is not limited
				...
				Lines[i].N=NumWords(L); //Parse number of words in the line
