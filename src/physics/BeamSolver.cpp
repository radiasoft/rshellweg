...
TError TBeamSolver::ParseLines(TInputLine *Lines,int N,bool OnlyParameters)
{
            ...
            case CELL:{
			   ...
			   //	Cells[Ni].Drift=false;
				...
				if (NewCell && !PowerDefined) {
					ShowMessage("ERROR: The RF power source must be defined before each RF section. Put POWER line in correct format into the input file before the end of each RF section. Note that DRIFT element terminates the defined RF power!");
					Error=ERR_FORMAT;
				}
				NewCell=false;
				dF=0;
			...
			case DUMP:{
				if (Lines[k].N>=1 && Lines[k].N<=3){
					...
					if (Cells[Ni].DumpParameters.N1<0 || Cells[Ni].DumpParameters.N2<0) {
						ShowMessage("WARNING: The particle numbers in SAVE line can not be negative. The region will be ignored.");
						Cells[Ni].DumpParameters.N1=0;
						Cells[Ni].DumpParameters.N2=0;
					...
					ShowMessage("WARNING: The format of SAVE line is wrong and will be ignored.");
				}
				break;
			}
			case COMMENT:{
   ...
   /*	if (CellDefined && !PowerDefined) {
		ShowMessage("ERROR: The RF power source must be defined before each RF section. Put POWER line in correct format into the input file before the end of each RF section");
		Error=ERR_FORMAT;
	}       */
...
int TBeamSolver::CreateBeam()
{
   ...
   /*   b0=MeVToVelocity(W0);
    ...
    if (BeamType==RANDOM) {
		...
		if (!Beam[0]->ReadCSTEmittance(BeamFile,Np)) {
            return ERR_CURRENT;
		}
    }
   ...
   //
	for (int i=0;i<Npoints;i++){
		...
		if (Structure[i].Dump) {
			...
			if (jmin==0 && jmax==Np) {
				fo<<"List of all living particles at z=";
			}  else {
				fo<<"List of living particles from #";
				fo<<jmin+1;
				fo<<" to #";
				fo<<jmax;
				fo<<" at z=";
