...
//---------------------------------------------------------------------------
double TBeamSolver::GetFrequency(int Nsec)
{
	return StructPar.Sections[Nsec].Frequency;
}
//---------------------------------------------------------------------------
double TBeamSolver::GetPower(int Nsec)
{
...
//---------------------------------------------------------------------------
TError TBeamSolver::ParseDump(TInputLine *Line, int Ni)
{
	...
	if (Line->N>=1 && Line->N<=9){
		...
		StructPar.Cells[Ni].DumpParameters.File=Line->S[0].c_str();
		...
		F+=Line->S[0];
			if (Line->N>=2 && IsNumber(Line->S[1])) {
				StructPar.Cells[Ni].DumpParameters.N1=Line->S[1].ToDouble();
				F=F+" "+Line->S[1];
				Nkey=2;
				if (Line->N>=3 && IsNumber(Line->S[2])){
					StructPar.Cells[Ni].DumpParameters.N2=Line->S[2].ToDouble();
					F=F+" "+Line->S[2];
					Nkey=3;
				} else
...
//---------------------------------------------------------------------------
TGauss TBeamSolver::GetEnergyStats(int Nknot)
{
	return Beam[Nknot]->GetEnergyDistribution();
}
//---------------------------------------------------------------------------
TGauss TBeamSolver::GetPhaseStats(int Nknot)
{
	return Beam[Nknot]->GetPhaseDistribution();
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
double TBeamSolver::GetWavelength(int Ns)
{
	return c/StructPar.Sections[Ns].Frequency;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
double TBeamSolver::GetBeamRadius(int Ni)
{
	return Beam[Ni]->GetRadius();
}
//---------------------------------------------------------------------------
...
TGraph *TBeamSolver::GetTrace(int Np,TBeamParameter P1,TBeamParameter P2)
{
	...
	for (int i = 0; i < Npoints; i++) {
		...
		G[i].draw=Beam[i]->GetParameter(Np,LIVE_PAR)==1;
	}
...
TGraph *TBeamSolver::GetSpace(int Nknot,TBeamParameter P1,TBeamParameter P2)
{
	...
	for (int i = 0; i < BeamPar.NParticles; i++) {
		...
		G[i].draw=Beam[Nknot]->GetParameter(i,LIVE_PAR)==1;
	}
		...
		if (Structure[i].Dump) {
			...
			for (int j=jmin;j<jmax;j++){
				if(!Structure[i].DumpParameters.LiveOnly || (Structure[i].DumpParameters.LiveOnly && !Beam[i]->Particle[j].lost)){
					...
					if (Structure[i].DumpParameters.Energy){
						s=s.FormatFloat("#0.00\t\t\t",VelocityToMeV(Beam[i]->Particle[j].beta));
						fo<<s.c_str();
					}
					if (Structure[i].DumpParameters.Radius){
						s=s.FormatFloat("#0.00\t\t\t",1000*Beam[i]->Particle[j].r*Structure[i].lmb);
						fo<<s.c_str();
					}
					if (Structure[i].DumpParameters.Azimuth){
					...
					if (Structure[i].DumpParameters.Vx){
						s=s.FormatFloat("#0.00\t\t",Beam[i]->Particle[j].Br*c);
						fo<<s.c_str();
					}
...
#endif
{
  ...
  /*  double A=0;
    ...
    Result.Length=z;
	Result.AverageEnergy=Gw.mean;
	Result.MaximumEnergy=Wm;
	Result.EnergySpectrum=Gw.sigma;
	Result.InputCurrent=1e3*I0;
    Result.BeamCurrent=1e3*I;
    Result.Captured=kc;
	Result.BeamRadius=r;
	Result.AveragePhase=RadToDegree(Gphi.mean);
	Result.PhaseLength=RadToDegree(Gphi.sigma);
    Result.BeamPower=P;
	Result.LoadPower=Pb;
	Result.Alpha=T.alpha;
	Result.Betta=100*T.beta;
	Result.Emittance=100*T.epsilon;
  //  Result.A=A;
